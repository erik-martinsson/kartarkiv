...
import { loadWinSplits } from "../../../../studio/src/lib/winsplits";

const EVENTOR_BASE_URL =
  "https://eventor.orientering.se";

export type WinSplitsInformation = {
  databaseId: number;
  categoryId: number;
  url: string;
  title: string;
...
export type DirectWinSplitsImport = {
  title: string;
  date: string;
  club: string;
  raceClass: string;
...
async function resolveEventorEventWithBrowser(
  browser: Browser,
  winsplitsUrl: string,
) {
  const context =
...
export type EventorResolution = Awaited<
  ReturnType<typeof resolveEventorEventWithBrowser>
>;

export async function resolveEventorFromWinSplits(
  winsplitsUrl: string,
): Promise<EventorResolution> {
  const browser = await chromium.launch({
    headless: true,
  });

  try {
    return await resolveEventorEventWithBrowser(
      browser,
      winsplitsUrl,
    );
  } finally {
