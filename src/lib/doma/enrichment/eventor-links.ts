...
function findLastClassInformation(
  ...
  const fullClassPattern =
    /(?:^|\s)([\p{L}\p{N}][\p{L}\p{N}\s/+().:_-]{0,60}?)\s+(\d[\d\s]*)\s*m\s*,?\s*(\d+)\s+startande/giu;
...
export async function getEventLinks(
  ...
  try {
    ...
    if (winsplits) {
      ...
      console.log(
        "[DEBUG] Erik-kandidater:",
        eriks,
      );

      runner = findRunnerByName(
        runners,
        runnerName,
      );
      ...
      if (!runner) {
        throw new Error(
          ...
          `WinSplits innehåller ${runners.length} löpare.`,
        );
      }
    }
